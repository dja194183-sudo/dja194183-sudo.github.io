const P=["BTC/USDT","ETH/USDT","BNB/USDT","SOL/USDT","XRP/USDT","DOGE/USDT","ADA/USDT","AVAX/USDT","LINK/USDT","SUI/USDT","PEPE/USDT","TON/USDT","ARB/USDT","NEAR/USDT","OP/USDT","APT/USDT","INJ/USDT","ATOM/USDT","LTC/USDT","FIL/USDT","WLD/USDT","TIA/USDT"];
const TF=["5m","15m","1h","4h"];
const TIPS=["wait for pullback","do not chase","WAIT is a position","risk 0.4%","no revenge"];
const S={sym:"BTC/USDT",mkt:"futures",tf:"15m",sig:null,edit:null,paper:[],best:[]};
const K={j:"khatoon:j",c:"khatoon:c",p:"khatoon:p",b:"khatoon:best"};
...
function hTrend(kl){const cl=(kl||[]).map(r=>+r[4]); const i=cl.length-1; const e21=ema(cl,21), e50=ema(cl,50); if(!e21[i]||!e50[i]) return 0; if(e21[i]>e50[i]&&cl[i]>e21[i]) return 1; if(e21[i]<e50[i]&&cl[i]<e21[i]) return -1; return 0}
...
function renderBest(){const box=document.getElementById("scanlist"); const st=document.getElementById("scanstat"); if(!box)return; const rows=S.best||[]; const t=S.bestAt? new Date(S.bestAt).toLocaleTimeString():""; st.textContent=rows.length? ("auto 15m+1h · "+rows.length+" · "+t):("no setup · "+t); box.innerHTML=rows.map(r=>`<div class="item"><button data-p="${r.sym}">${r.sym}</button> <b class="${r.d}">${r.d}</b> ${r.sc}% <span class="hint">${r.ht||""}</span><div class="hint">${fmt(r.px)} SL ${fmt(r.sl)} TP ${fmt(r.tp)}</div></div>`).join("")||"<p class=hint>WAIT on all pairs</p>"}
async function scanBest(){const st=document.getElementById("scanstat"); if(st) st.textContent="scanning 22 pairs..."; const out=[]; for(const pair of P){ try{ const s=pair.replace("/",""); const tk=await get("https://data-api.binance.vision/api/v3/ticker/24hr?symbol="+s); const kl=await get("https://data-api.binance.vision/api/v3/klines?symbol="+s+"&interval=15m&limit=180"); const h1=await get("https://data-api.binance.vision/api/v3/klines?symbol="+s+"&interval=1h&limit=80"); const px=Number(tk.lastPrice||tk.c); const cs=(kl||[]).map(r=>({o:+r[1],h:+r[2],l:+r[3],c:+r[4]})); const sig=analyze(cs,px); const ht=hTrend(h1); if(sig.d==="LONG"&&ht<0) continue; if(sig.d==="SHORT"&&ht>0) continue; if(sig.d!=="WAIT") out.push({sym:pair,d:sig.d,sc:sig.sc+(ht!==0?6:0),px,sl:sig.sl,tp:sig.tp,ht:ht>0?"1H+":ht<0?"1H-":"1H~"}); }catch(e){} } out.sort((a,b)=>b.sc-a.sc); S.best=out; S.bestAt=Date.now(); try{localStorage.setItem(K.b,JSON.stringify({at:S.bestAt,rows:out}))}catch(e){} renderBest();}
...
const rs=document.getElementById("rescan"); if(rs) rs.onclick=()=>scanBest();
...
try{const b=JSON.parse(localStorage.getItem(K.b)||"{}"); if(b.rows){S.best=b.rows; S.bestAt=b.at; renderBest()}}catch(e){}
drawPairs(); drawTf(); showC(); load(); scanBest(); setInterval(load,8000); setInterval(scanBest,90000);
